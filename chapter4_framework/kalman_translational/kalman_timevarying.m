%% Predictive translational motion control (3D) - time-varying motion
% By: Joris van Gool
...
% External force (polynomial)
a0 = 0.1; a1 = 0.01; a2 = -0.001;
f1 = a0 + a1*time + a2*time.^2;
f_ext = [f1 .* (time >= T/2); zeros(5, N)];
...
x_est = [p(1); p(2); zeros(4,1)];  % Initial state
P_est = eye(6);                    % Initial covariance matrix
...
Q = diag([0.1, 0.1, 1, 1, 1, 1]); % Process noise
