...
# Formation control node ArUco -drone 2
# Last edited: 18/06/2025
# Author: Joris van Gool

print("#######################################################")
print("#    Formation control node ArUco - drone 2           #")
...
def aruco_cb(msg):
    ...
    try:
        ...
        rospy.logwarn(f"Failed to parse /drone2/aruco_data: {e}")
...
def main():
    ...
    rospy.Subscriber("/drone2/mavros2/local_position/pose", PoseStamped, pose_cb)
    rospy.Subscriber("/drone2/aruco_data", String, aruco_cb)
    ...
    d = np.array([np.sqrt(16), np.sqrt(13), np.sqrt(13)])
