%% Predictive translational motion control (3D) - complex sinusoidal motion
% By: Joris van Gool
...
% Random parameters for x-direction
a0_x = 0.5; a1_x = -0.01; a2_x = 0.0002;
alpha_x = 2.0; beta_x = 1.5; omega1 = 1.1;

% Random parameters for y-direction
a0_y = -0.3; a1_y = 0.02; a2_y = -0.0001;
alpha_y = 1.2; beta_y = -0.8; omega2 = 1.4;

% Construct velocity signals
fx = a0_x + a1_x * time + a2_x * time.^2 + alpha_x * sin(omega1 * time) + beta_x * cos(omega1 * time);
fy = a0_y + a1_y * time + a2_y * time.^2 + alpha_y * sin(omega2 * time) + beta_y * cos(omega2 * time);

% Apply external force only after t >= T/2
f_ext = [fx .* ((time >= T/10*4) & (time < T/10*7));
         fy .* ((time >= T/10*4) & (time < T/10*7));
         zeros(4, N)]; 
...
% Define time span to show
idx_end = floor(length(time) / 1);
...
function [x_est, P_est] = kalman_filter(x_est, P_est, z, dt, t)
    ...
    % Noise matrices
    Q = diag([1e-4*ones(1,2), 1e-2*ones(1,2), 1e-1*ones(1,6), 1e-2*ones(1,4), 1e-3*ones(1,2)]);
