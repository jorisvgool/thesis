%% Predictive rotational motion with virtual control framework (2D) - constant gamma
% By: Joris van Gool
...
for t = 1:N
    if t <= N/2
        ...
        alpha = (t - N/2) / (N - N/2);  % goes from 0 to 1
        theta = ang_start + alpha * (ang_end - ang_start);
    end
...
% Initialize Kalman
x_est = [ang_start; 0];  % Initial state
P_est = eye(2);          % Initial covariance matrix
Rk = 0 * eye(1); 
W = zeros(1,N);
G = zeros(1,N);
...
Q = diag([0.1, 1]); % Process noise
R = 1e-3 * eye(1);  % Measurement noise
